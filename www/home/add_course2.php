...
if(!isset($_POST['course']) || !isset($_POST['section']) || !isset($_POST['section_capacity']) || !isset($_POST['start_time']) || !isset($_POST['end_time']) || !isset($_POST['building']) || !isset($_POST['room']) || !isset($_POST['room_capacity']) || !isset($_POST['enrollment']) || !isset($_POST['ta_grader']) || !isset($_POST['ta_hour']) || !isset($_POST['catalog_year']) || !isset($_POST['academic_year'])) {
	...
	$sth = $mysql->prepare("INSERT INTO `courses` (`course`, `section`, `course_title`, `section_capacity`, `start_time`, `end_time`, `building`, `room`, `room_capacity`, `enrollment`, `ta_grader`, `ta_hours`, `catalog_year`, `academic_year`, `semester`) VALUES (:course, :section, :title, :section_capacity, :start_time, :end_time, :building, :room, :room_capacity, :enrollment, :ta_grader, :ta_hours, :catalog_year, :academic_year, :semester)");

	$check = $sth->execute(array("course" => $_POST['course'], "section" => $_POST['section'], "title" => $_POST['title'], "section_capacity" => $_POST['section_capacity'], "start_time" => $_POST['start_time'], "end_time" => $_POST['end_time'], "building" => $_POST['building'], "room" => $_POST['room'], "room_capacity" => $_POST['room_capacity'], "enrollment" => $_POST['enrollment'], "ta_grader" => $_POST['ta_grader'], "ta_hours" => $_POST['ta_hour'], "catalog_year" => $_POST['catalog_year'], "academic_year" => $_POST['academic_year'], "semester" => $_POST['semester']));
	...
	if($check) {
		...
		foreach($_POST['instructors'] as $i) {
			...
			$check1 = $instructor_add->execute(array("user_id" => $instructor->user_id, "course_id" => $course_id));
